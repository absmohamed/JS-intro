...
- [Express.js](#expressjs)
  ...
  - [Setting up for connection to our server](#setting-up-for-connection-to-our-server)
  - [Display a list of students](#display-a-list-of-students)
  - [Adding a student to the list](#adding-a-student-to-the-list)
  - [Challenge](#challenge)
...
_What kind of request do we need to send to the server to get a list of students?_

We need to send a `GET` request on the `studentsUrl`.

We can use `fetch` to do this. Remember that `fetch` _returns a Promise_. We will have to resolve the promise, and call a function with the result to display the list of students.
...
_What should we do when the promise resolves?_
...
When we test this, we can see the logged message that the student was added, but we don't see the list updated. _Why?_
...
Oh-oh... _What happened? How can we fix it?_
